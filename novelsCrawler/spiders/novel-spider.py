...
class NovelSpider(scrapy.Spider):
    ...
    @abc.abstractmethod
    def parse_title(self, selector):
        return ''

    @abc.abstractmethod
    def parse_episoders(self, selector):
        return []

    @abc.abstractmethod
    def parse_content(self, selector):
        return []

    def parse(self, response):
        sel = Selector(response)
        title = self.parse_title(selector=sel)
        ...
        episoders = self.parse_episoders()
    ...
    def parse_page(self, response):
        ...
        item['content'] = self.parse_content(selector=sel)
        return item
