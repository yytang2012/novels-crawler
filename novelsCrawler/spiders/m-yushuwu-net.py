...
class YushuwuNetMobileSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if start_page_key in response.meta:
            startPage = response.meta[start_page_key]
        else:
            startPage = 1
        if index_key in response.meta:
            pageIndex = response.meta[index_key]
        else:
            pageIndex = []
        ...
        all_pages = [i + startPage for i in range(0, len(subtitle_selectors))]
        pageIndex += all_pages
        ...
        if next_page_url != 'javascript:void(0);':
            ...
            request.meta[start_page_key] = len(subtitle_selectors) + startPage
            request.meta[title_key] = title
            request.meta[index_key] = pageIndex
            yield request
        else:
            save_index(title, response.url, tmp_spider_root_dir, pageIndex)
