...
class LwxslwxsSpider(NovelSpider):
    """
    classdocs

    example: https://www.lwxslwxs.com/46/46292/
    """
    ...
    def parse_episodes(self, response):
        sel = Selector(response)
        episodes = []
        tmp = sel.xpath('//div[@class="panel-body"]')[-1]
        subtitle_selectors = tmp.xpath('ul[@class="list-group list-charts"]/li/a')
    ...
    def parse_content(self, response):
        sel = Selector(response)
        content = sel.xpath('//div[@id="content-txt"]/text()').extract()
